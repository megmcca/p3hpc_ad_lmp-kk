...
t=1000
echo "sizestr x y z natoms loop-time timesteps/s Matom*step/s neigh-thread neigh-transpose" > $txt
...
thread=on
trans=on
lmplog=log.lammps.date=${lmpdate}.model=${model}.machine=${machine}.pkg=kokkos_gpu.kind=node.size=${sizestr}.node=1.mpi=1.gpu=1.mode=off.neighthread=${thread}
mpirun -np 1 --bind-to core  ${lmpexe} -sf kk -k on g 1 -pk kokkos neigh full neigh/qeq full neigh/thread ${thread} neigh/transpose ${trans} newton off comm device pair/only off -v x $x -v y $y -v z $z -v t $t -in ${infile} -nocite -log ${lmplog}
stats=`parse_log $lmplog`
echo $sizestr $x $y $z $stats $thread $trans >> $txt
